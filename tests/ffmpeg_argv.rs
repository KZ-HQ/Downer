...
fn argv_for(case: Case) -> Vec<String> {
    ...
    let options = DownloadOptions {
        ...
        // This suite asserts on one rendered command line; a retry would
        // overwrite the recording with a later attempt's.
        retries: 0,
...
fn direct_file() {
    assert_eq!(
        ...
        [
            ...
            "error",
            "-stats",
            // Reconnection is on for every HTTP(S) input (KEI-66). The
            // statuses are deliberately not `4xx,5xx`: a 403 or a 404 will not
            // become a 200, so reconnecting on all of 4xx would hammer a server
            // that has already answered. See ADR-0023.
            "-reconnect",
            "1",
            "-reconnect_streamed",
            "1",
